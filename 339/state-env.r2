

     ;;
    set -o posix ;; #(
  # Pre-4.2 versions of Zsh do word splitting on ${1+"$@"}, which
  # is contrary to our usage.  Disable this feature.
  *) :
  *posix*) :
  NULLCMD=:
  alias -g '\''${1+"$@"}'\''='\''"$@"'\''
  as_lineno_2=196 as_lineno_2a=$LINENO
  case `(set -o) 2>/dev/null` in #(
  emulate sh
  eval '\''test "x$as_lineno_1'\''$as_run'\''" != "x$as_lineno_2'\''$as_run'\''" &&
  exitcode=1; echo positional parameters were not saved.
  setopt NO_GLOB_SUBST
  test "x`expr $as_lineno_1'\''$as_run'\'' + 1`" = "x$as_lineno_2'\''$as_run'\''"'\'' || exit 1'
'
'
'
'
'
'
AUTOM4TE=autom4te
Apple_PubSub_Socket_Render=/tmp/launch-KxfPtt/Render
Apple_Ubiquity_Message=/tmp/launch-KGv0zg/Apple_Ubiquity_Message
BASH=/bin/sh
BASH_ARGC=([0]="1")
BASH_ARGV=([0]="-C")
BASH_LINENO=([0]="0")
BASH_SOURCE=([0]="./configure")
BASH_VERSINFO=([0]="3" [1]="2" [2]="48" [3]="1" [4]="release" [5]="x86_64-apple-darwin11")
BASH_VERSION='3.2.48(1)-release'
CC
CC
CC=/usr/bin/llvm-gcc
CFLAGS
CFLAGS
CFLAGS='-Os -w -pipe -march=core2 -msse4'
CLICOLOR=true
COMMAND_MODE=unix2003
CONFIG_STATUS=./config.status
CPPFLAGS
CPPFLAGS'
CXX=/usr/bin/llvm-g++
CXXFLAGS='-Os -w -pipe -march=core2 -msse4'
DEFS
DEFS=-DHAVE_CONFIG_H
DIRSTACK=()
DISPLAY=/tmp/launch-Q8qWpT/org.x:0
DUALCASE=1
ECHO_C
ECHO_C='\c'
ECHO_N
ECHO_N=
ECHO_T
ECHO_T=
EDITOR=vi
EUID=501
EXEEXT
EXEEXT=
FC
FC
FC=gfortran
FCFLAGS
FCFLAGS
FCFLAGS='-g -O2'
FCLIBS
FCLIBS=' -L/usr/local/Cellar/gfortran/4.2.4-5666.3/bin/../lib/gcc/i686-apple-darwin11/4.2.1/x86_64 -L/usr/local/Cellar/gfortran/4.2.4-5666.3/bin/../lib/gcc/i686-apple-darwin11/4.2.1 -L/usr/local/Cellar/gfortran/4.2.4-5666.3/bin/../lib/gcc -L/usr/local/Cellar/gfortran/4.2.4-5666.3/bin/../lib/gcc/i686-apple-darwin11/4.2.1/../../.. -lgfortranbegin -lgfortran'
FC_DUMMY_MAIN=none
GCC=yes
GEM_HOME=/usr/local
GFC=yes
GROUPS=()
HOME=/Users/nibbles
HOMEBREW_BREW_FILE=/usr/local/bin/brew
HOMEBREW_ERROR_PIPE=4
HOMEBREW_MAKE_JOBS=3
HOSTNAME=LeBus.local
HOSTTYPE=x86_64
IFS=' 	
LANG=en_US.UTF-8
LANGUAGE=C
LC_ALL=C
LD=/usr/bin/llvm-gcc
LDFLAGS
LDFLAGS
LIBOBJS
LIBOBJS=
LIBS
LIBS
LINENO=3768
LOGNAME=nibbles
LTLIBOBJS=
MACHTYPE=x86_64-apple-darwin11
MAKEFLAGS=
MAKELEVEL=4
MANPATH=/usr/local/share/man:/usr/share/man:/usr/X11/share/man
MFLAGS=
OBJCFLAGS='-Os -w -pipe -march=core2 -msse4'
OBJCXXFLAGS='-Os -w -pipe -march=core2 -msse4'
OBJEXT
OBJEXT=o
OPTERR=1
OPTIND=1
OSTYPE=darwin11
PACKAGE_BUGREPORT
PACKAGE_BUGREPORT=
PACKAGE_NAME
PACKAGE_NAME=
PACKAGE_STRING
PACKAGE_STRING=
PACKAGE_TARNAME
PACKAGE_TARNAME=
PACKAGE_URL
PACKAGE_URL=
PACKAGE_VERSION
PACKAGE_VERSION=
PATH=/private/tmp/homebrew-autoconf-2.68b-FNje/autoconf-2.68b/tests:/usr/local/bin:/usr/local/share/python:/usr/local/texlive/2011/bin/x86_64-darwin:/usr/bin:/bin:/usr/sbin:/sbin:/usr/X11/bin:/usr/local/Library/Contributions/examples
PATH_SEPARATOR
PATH_SEPARATOR=:
PIPESTATUS=([0]="1")
POSIXLY_CORRECT=y
PPID=51613
PS1='$ '
PS2='> '
PS4='+ '
PWD=/private/tmp/homebrew-autoconf-2.68b-FNje/autoconf-2.68b/tests/testsuite.dir/339
PYTHONPATH=/usr/local/lib/python2.7/site-packages
SECURITYSESSIONID=186a4
SHELL'
SHELL=/bin/sh
SHELLOPTS=braceexpand:hashall:interactive-comments:posix
SHLVL=4
SSH_AUTH_SOCK=/tmp/launch-p4zt4y/Listeners
TAR_OPTIONS='--owner=0 --group=0 --numeric-owner'
TERM=xterm-256color
TERM_PROGRAM=Apple_Terminal
TERM_PROGRAM_VERSION=303
TERM_SESSION_ID=F7E807D1-C6BB-48F6-8B9B-525E64A78748
TMPDIR=/var/folders/02/8j4kgb3s6snglmmgq25h3ypw0000gn/T/
U=
UID=501
USER=nibbles
_=
__CF_USER_TEXT_ENCODING=0x1F5:0:0
ac_abs_confdir=/private/tmp/homebrew-autoconf-2.68b-FNje/autoconf-2.68b/tests/testsuite.dir/339
ac_arg='CFLAGS=-Os -w -pipe -march=core2 -msse4'
ac_aux_dir=.
ac_cache_corrupted=false
ac_clean_files=
ac_clean_files_save=
ac_compile='$CC -c $CFLAGS $CPPFLAGS conftest.$ac_ext >&5'
ac_compiler='$CC'
ac_compiler_gnu=yes
ac_confdir=.
ac_config_guess='/bin/sh ./config.guess'
ac_config_headers=' config.h:config.hin'
ac_config_libobj_dir=.
ac_config_status_args=
ac_config_sub='/bin/sh ./config.sub'
ac_configure='/bin/sh ./configure'
ac_configure_args=' '\''-C'\'' '\''CC=/usr/bin/llvm-gcc'\'' '\''CFLAGS=-Os -w -pipe -march=core2 -msse4'\'''
ac_cpp='$CPP $CPPFLAGS'
ac_cs_success=:
ac_ct_CC
ac_ct_CC=/usr/bin/llvm-gcc
ac_ct_FC
ac_ct_FC=gfortran
ac_cv_build=x86_64-apple-darwin11.3.0
ac_cv_c_compiler_gnu=yes
ac_cv_env_CC_set=set
ac_cv_env_CC_value=/usr/bin/llvm-gcc
ac_cv_env_CFLAGS_set=set
ac_cv_env_CFLAGS_value='-Os -w -pipe -march=core2 -msse4'
ac_cv_env_CPPFLAGS_set=
ac_cv_env_CPPFLAGS_value=
ac_cv_env_FCFLAGS_set=
ac_cv_env_FCFLAGS_value=
ac_cv_env_FC_set=
ac_cv_env_FC_value=
ac_cv_env_LDFLAGS_set=
ac_cv_env_LDFLAGS_value=
ac_cv_env_LIBS_set=
ac_cv_env_LIBS_value=
ac_cv_env_build_alias_set=
ac_cv_env_build_alias_value=
ac_cv_env_host_alias_set=
ac_cv_env_host_alias_value=
ac_cv_env_target_alias_set=
ac_cv_env_target_alias_value=
ac_cv_fc_compiler_gnu=yes
ac_cv_fc_dummy_main=none
ac_cv_fc_libs=' -L/usr/local/Cellar/gfortran/4.2.4-5666.3/bin/../lib/gcc/i686-apple-darwin11/4.2.1/x86_64 -L/usr/local/Cellar/gfortran/4.2.4-5666.3/bin/../lib/gcc/i686-apple-darwin11/4.2.1 -L/usr/local/Cellar/gfortran/4.2.4-5666.3/bin/../lib/gcc -L/usr/local/Cellar/gfortran/4.2.4-5666.3/bin/../lib/gcc/i686-apple-darwin11/4.2.1/../../.. -lgfortranbegin -lgfortran'
ac_cv_fc_mangling='lower case, underscore, no extra underscore'
ac_cv_fortran_dummy_main=none
ac_cv_host=x86_64-apple-darwin11.3.0
ac_cv_objext=o
ac_cv_prog_ac_ct_CC=/usr/bin/llvm-gcc
ac_cv_prog_ac_ct_FC=gfortran
ac_cv_prog_cc_c89=
ac_cv_prog_cc_g=yes
ac_cv_prog_fc_g=yes
ac_cv_prog_fc_v=-v
ac_dashdash=
ac_default_prefix=/usr/local
ac_delim='%!_!# '
ac_dir=.
ac_env_CC_set=set
ac_env_CC_value=/usr/bin/llvm-gcc
ac_env_CFLAGS_set=set
ac_env_CFLAGS_value='-Os -w -pipe -march=core2 -msse4'
ac_env_CPPFLAGS_set=
ac_env_CPPFLAGS_value=
ac_env_FCFLAGS_set=
ac_env_FCFLAGS_value=
ac_env_FC_set=
ac_env_FC_value=
ac_env_LDFLAGS_set=
ac_env_LDFLAGS_value=
ac_env_LIBS_set=
ac_env_LIBS_value=
ac_env_build_alias_set=
ac_env_build_alias_value=
ac_env_host_alias_set=
ac_env_host_alias_value=
ac_env_target_alias_set=
ac_env_target_alias_value=
ac_exeext=
ac_ext=c
ac_file=conftest
ac_files='a.out conftest.exe conftest a.exe a_out.exe b.out conftest.*'
ac_hostname=LeBus.local
ac_i=:
ac_init_help=
ac_init_version=false
ac_install_sh='./install-sh -c'
ac_last_try=false
ac_libobjs=
ac_link='$CC -o conftest$ac_exeext $CFLAGS $CPPFLAGS $LDFLAGS conftest.$ac_ext $LIBS >&5'
ac_link_default='$FC $FCFLAGS $LDFLAGS $ac_fcflags_srcext conftest.$ac_ext $LIBS >&5'
ac_ls_di='20020132 .'
ac_ltlibobjs=
ac_msg='sources are in ., but `cd .'\'' does not work'
ac_must_keep_next=false
ac_new_set=
ac_new_val=
ac_objext=o
ac_old_set=
ac_old_val=
ac_optarg=yes
ac_option=-qversion
ac_pass=2
ac_precious_vars='build_alias
ac_prev=
ac_prog=gfortran
ac_pwd=/private/tmp/homebrew-autoconf-2.68b-FNje/autoconf-2.68b/tests/testsuite.dir/339
ac_pwd_ls_di='20020132 .'
ac_rmfiles=' a.out conftest.exe conftest a.exe a_out.exe b.out'
ac_save_CFLAGS='-Os -w -pipe -march=core2 -msse4'
ac_save_FCFLAGS=
ac_save_IFS=' 	
ac_save_ext=f
ac_signal=0
ac_site_file1=/usr/local/share/config.site
ac_site_file2=/usr/local/etc/config.site
ac_site_file=/usr/local/etc/config.site
ac_srcdir_defaulted=yes
ac_status=1
ac_subst_files=
ac_subst_vars='LTLIBOBJS
ac_test_CFLAGS=set
ac_test_FCFLAGS=
ac_tool_prefix=
ac_try='$CC -qversion >&5'
ac_try_echo='configure:3050: /usr/bin/llvm-gcc -qversion >&5'
ac_tt=
ac_unrecognized_opts=
ac_unrecognized_sep=
ac_user_opts='
ac_val='${datarootdir}/man'
ac_var=CPPFLAGS
ac_word=gcc
ac_word_re='[_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ][_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789]*'
ac_write_fail=0
as_basename=basename
as_bourne_compatible='if test -n "${ZSH_VERSION+set}" && (emulate sh) >/dev/null 2>&1; then :
as_cr_LETTERS=ABCDEFGHIJKLMNOPQRSTUVWXYZ
as_cr_Letters=abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ
as_cr_alnum=abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789
as_cr_digits=0123456789
as_cr_letters=abcdefghijklmnopqrstuvwxyz
as_dir=/usr/local/Library/Contributions/examples
as_dirname=dirname
as_echo='printf %s\n'
as_echo_n='printf %s'
as_executable_p=as_fn_executable_p
as_expr=false
as_fn_failure && { exitcode=1; echo as_fn_failure succeeded.; }
as_fn_failure () { as_fn_return 1; }
as_fn_ret_failure && { exitcode=1; echo as_fn_ret_failure succeeded.; }
as_fn_ret_failure () { return 1; }
as_fn_ret_success () { return 0; }
as_fn_ret_success || { exitcode=1; echo as_fn_ret_success failed.; }
as_fn_success () { as_fn_return 0; }
as_fn_success || { exitcode=1; echo as_fn_success failed.; }
as_have_required=yes
as_lineno_1=464
as_lineno_1a=464
as_lineno_2=465
as_lineno_2a=465
as_ln_s='ln -s'
as_me=configure
as_mkdir_p='mkdir -p "$as_dir"'
as_myself=./configure
as_nl='
as_required='as_fn_return () { (exit $1); }
as_save_IFS=' 	
as_suggested='  as_lineno_1=195 as_lineno_1a=$LINENO
as_test_x='test -x'
as_tr_cpp='eval sed '\''y%*abcdefghijklmnopqrstuvwxyz%PABCDEFGHIJKLMNOPQRSTUVWXYZ%;s%[^_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789]%_%g'\'''
as_tr_sh='eval sed '\''y%*+%pp%;s%[^_abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789]%_%g'\'''
as_unset=as_fn_unset
as_var=MAILPATH
as_write_fail=0
bindir
bindir='${exec_prefix}/bin'
build
build=x86_64-apple-darwin11.3.0
build_alias
build_cpu
build_cpu=x86_64
build_os
build_os=darwin11.3.0
build_vendor
build_vendor=apple
cache_file=config.cache
containing=
cross_compiling=no
datadir
datadir='${datarootdir}'
datarootdir
datarootdir='${prefix}/share'
docdir
docdir='${datarootdir}/doc/${PACKAGE}'
dvidir
dvidir='${docdir}'
else
else
enable_option_checking
esac
exclude=
exec_prefix
exec_prefix='${prefix}'
exitcode=0
fi
fi
halt=
host
host=x86_64-apple-darwin11.3.0
host_alias
host_alias
host_cpu
host_cpu=x86_64
host_os
host_os=darwin11.3.0
host_vendor
host_vendor=apple
htmldir
htmldir='${docdir}'
if ( set x; as_fn_ret_success y && test x = "$1" ); then :
ignore_case=
in_files=
in_vc_files=
includedir
includedir='${prefix}/include'
infodir
infodir='${datarootdir}/info'
libdir
libdir='${exec_prefix}/lib'
libexecdir
libexecdir='${exec_prefix}/libexec'
localedir
localedir='${datarootdir}/locale'
localstatedir
localstatedir='${prefix}/var'
mandir
mandir='${datarootdir}/man'
no_create=
no_recursion=
non_containing=
oldincludedir
oldincludedir=/usr/include
pdfdir
pdfdir='${docdir}'
prefix
prefix=/usr/local
program_prefix=NONE
program_suffix=NONE
program_transform_name
program_transform_name=s,x,x,
prohibit=
psdir
psdir='${docdir}'
require=
sbindir
sbindir='${exec_prefix}/sbin'
sharedstatedir
sharedstatedir='${prefix}/com'
silent=
site=
srcdir=.
subdirs=
sysconfdir
sysconfdir='${prefix}/etc'
target=
target_alias
target_alias
test -x / || exit 1'
test x$exitcode = x0 || exit 1
verbose=
with_grep_options=
x_includes=NONE
x_libraries=NONE
